...
#bgbox {
	...
	left: 140px;
	top: 40px;
	background-image: url(images/manage/delete/delete/delete-bg_03.png);
}
...
<%DB db=new DB();
  ...
  <div id="apDiv6">
   <select name="movie" id="dd"><%
			  while(rs.next()){	%>
<Option value="<%=rs.getString(1) %>">《<%=rs.getString(2) %>》，<%=rs.getString(3) %>，<%=rs.getString(4)%></Option>
			<%}%>
			</Select>
   </div>
  <div id="apDiv5"><input type="image" src="images/manage/manage/enter-bt-on.png" name="shanchu"></div>
</form>
